...
#include "wifi.h"
#include <stm32f7xx_hal.h> //stm 32 drivers for gpio
#include <cmsis_os.h>
#include "usb_device.h"

extern osSemaphoreId_t binarySemTuyaHandle;
uint8_t controlChangeFlag = 0;
extern struct switchData SwitchDatas[10];
//void initmSwitchData();
//void initmSwitchData(){
...
//	}
//}
//struct switchState SwitchState = ;

//initmSwitchData();

extern void tuyaUart(unsigned char value);
...
static unsigned char dp_download_switch_1_handle(const unsigned char value[], unsigned short length)
{
    ...
    CDC_Transmit_FS("got sm",6);
    switch_1 = mcu_get_dp_download_bool(value,length);
    controlChangeFlag = 1;
    if(switch_1 == 0) {
    	SwitchDatas[0].state = false;
    	HAL_GPIO_WritePin(GPIOG, GPIO_PIN_6, GPIO_PIN_RESET);
    }else {
    	SwitchDatas[0].state = true;
...
void pollingRoutine(){
	if(controlChangeFlag) {
		osSemaphoreRelease(binarySemTuyaHandle);
		controlChangeFlag = 0;
	}
}
/*****************************************************************************
...
Function description : on DPID_LIGHT_MODE processing function
Input parameter : value:Source data
        : length:Data length
Return parameter : Successful return:SUCCESS/Failed to return:ERROR
Instructions for use : 可下发可上报type,need to report the result to App after data is dealt with
...
Function description : on DPID_SWITCH_INCHING processing function
Input parameter : value:Source data
        : length:Data length
Return parameter : Successful return:SUCCESS/Failed to return:ERROR
Instructions for use : 可下发可上报type,need to report the result to App after data is dealt with
...
Function description : on DPID_BACKLIGHT processing function
Input parameter : value:Source data
        : length:Data length
Return parameter : Successful return:SUCCESS/Failed to return:ERROR
Instructions for use : 可下发可上报type,need to report the result to App after data is dealt with
...
Function description : on DPID_COLOUR processing function
Input parameter : value:Source data
        : length:Data length
Return parameter : Successful return:SUCCESS/Failed to return:ERROR
Instructions for use : 可下发可上报type,need to report the result to App after data is dealt with
